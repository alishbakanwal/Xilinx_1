...
proc ::add_channel {chan url} {
	...
	foreach service [::tcf::get_services $chan] {
		...
		RunControl {
			...
			add_contexts [lindex \$reply 0] RunControl:context RunControl:children
		    }]

		    ::tcf::on_event $chan RunControl contextChanged [subst -noc {
			set reply [::tcf::read {$chan} a{o{}}]
			change_contexts [lindex \$reply 0] RunControl:context
		    }]

		    ::tcf::on_event $chan RunControl contextRemoved [subst -noc {
			set reply [::tcf::read {$chan} a{s}]
			remove_contexts [lindex \$reply 0] RunControl:children {RunControl:context RunControl:children RunControl:state StackTrace:context StackTrace:children \
										Expressions:children Expressions:context}
			dispose_exprs {$chan} [lindex \$reply 0] "local"
		    }]
