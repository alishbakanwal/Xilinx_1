namespace eval ::hsi::utils {
	...
	if { ![dict exists $designtable $hdf] } {
	    set design [::hsi::open_hw_design $hdf]
	    ...
	    if { $configurablecell != "" } {
		if { [check_ps_init_files $configurablecell $hdf] == 0 } {
...
EXAMPLE {
    ...
    getaddrmap system.hdf ps7_cortexa9_0
        Return the address map of peripherals connected to ps7_cortexa9_0.
        system.hdf is the hw specification file exported from Vivado.
}
}
...
SYNOPSIS {
    repo [OPTIONS]
        ...
        This command can be used to scan the repositories, to get the list
...
RETURNS {
    ...
    <none>
        List of paramters and description of each parameter of processor,
...
EXAMPLE {
    ...
    ::xsdb::setcmdmeta setlib brief {Set library & version in BSP}
...
EXAMPLE {
    ...
    #---------------------------------------------------------------------------------------#
    proc get_hw { hw } {
	if { [file extension $hw] != ".hdf" && [file extension $hw] != ".xml" } {
	    ...
	    set retfile "[::sdk::getws]/$hw/system.hdf"
	    if { ![file exists $retfile] } {
		set retfile "[::sdk::getws]/$hw/system.xml"
		if { ![file exists $retfile] } {
		    error "unable to find hardware specification file $hw"
		}
	    }
	    return $retfile
	} else {
		...
		error "unable to find hardware specification file/project $hw"
	    }
	}
    }
	...
	if { [file extension $sw] != ".mss"} {
		...
		return "[::sdk::getws]/$sw/system.mss"
	} else {
	    if { [file isfile $sw] } {
		return $sw
	    } else {
	...
	if { [file extension $sw] != ".mss"} {
	    ...
	    set proj "[::sdk::getws]/$sw/.sdkproject"
	} else {
	    if { [file isfile $sw] } {
    ...
    #---------------------------------------------------------#
    proc check_ps_init_files { args } {
	set ps [lindex $args 0]
	set hdf [lindex $args 1]
	set ps_cell [::common::get_property IP_NAME [::hsi::get_cells $ps -hierarchical]]
	set dirname [file dirname $hdf]

	if { $ps_cell == "processing_system7" } {
	    set ps_files { ps7_init.c ps7_init.h ps7_init.tcl }
	} else {
	    set ps_files { psu_init.c psu_init.h psu_init.tcl }
	}
	...
	foreach procname $proc_list {
		...
		set desc [string_format $params(json) [::common::get_property DESCRIPTION $os]]
	...
	foreach lib $liblist {
	    ...
	    set desc [string_format $params(json) [::common::get_property DESCRIPTION $lib]]
		...
		if { $peri == "ps7_cortexa9" || $peri == "psu_cortexa53" || $peri == "psu_cortexr5" || $peri == "psu_pmu" || $peri == "microblaze" } {
		    lappend supp_proc $peri
		}
	    }
	    if { [llength $supp_proc] == 0 } {
		set supp_proc {ps7_cortexa9 psu_cortexa53 psu_cortexr5 psu_pmu microblaze}
	    }
	    regsub -all {\\} $repo / repo
	...
	foreach driver $driverlist {
	    ...
	    set desc [string_format $params(json) [::common::get_property DESCRIPTION $driver]]
	...
	foreach peri $perilist {
		...
		foreach supp_peri $supp_peri_list {
			...
			foreach driver $driverlist {
			    ...
			    dict set drvdict $driver [dict_format $params(json) [dict create name $drvname version $drvver repo $drvrepo]]
			}
		    }
		}
	    }
	...
	foreach peri $perilist {
		...
		if { $regenabled != "false"} {
		    ...
		    set regdesc [string_format $params(json) [::common::get_property DESCRIPTION $reg]]
			...
			set f_desc [string_format $params(json) [::common::get_property DESCRIPTION $field]]
		    ...
		    dict set regdict $regname [dict_format $params(json)  [dict create description $regdesc address_offset $regaddr \
									   access $regaccess size $regsize \
									   fields [dict_format $params(json) $fielddict]]]
		}
	    }
	...
	set os_apps [source_all_apps]

	# Find all the files with TCL extensions in both standalone and linux directories
	dict for {app app_details} $os_apps {
	    set os_val [dict get $app_details os]
	    set path_val [dict get $app_details path]
	    ...
	    set userdefname [string_format $params(json) [namespace eval ::xsdb::sdk::$app {swapp_get_name}]]
		...
		set supp_processors [string_format $params(json) "ps7_cortexa9 psu_cortexa53 psu_cortexr5 microblaze psu_pmu"]
	    } else {
	    ...
	    set path [string_format $params(json) $path_val]

	    # Dict of Applications (KEY = app name & VALUE = dict of app information extracted above)
	    dict set retdict $app [dict_format $params(json) [dict create userdefname $userdefname description $desc \
							      supp_proc $supp_processors supp_os $supp_os \
							      path $path os [string_format $params(json) $os_val]]]
	}
	# Peripheral Test
	set userdefname [string_format $params(json) "Peripheral Tests"]
	set desc [string_format $params(json) "Simple test routines for all peripherals in the hardware."]
	set supp_processors [string_format $params(json) "ps7_cortexa9 psu_cortexa53 psu_cortexr5 microblaze"]
	set supp_os [string_format $params(json) "standalone xilkernel"]
	set path [string_format $params(json) ""]
	set osname [string_format $params(json) "standalone"]

	# Dict of Applications (KEY = app name & VALUE = dict of app information extracted above)
	dict set retdict "peripheral_tests" [dict_format $params(json) [dict create userdefname $userdefname description $desc \
									supp_proc $supp_processors supp_os $supp_os \
									path $path os $osname]]
	...
	# Install path
	dict set app_repo standalone "$sdk/data/embeddedsw/lib/sw_apps/"

	# Get apps directories from all the added repositories
	set app_repos [get_all_repos $repopath $app_repo]

	set sw_apps_dirs [dict get $app_repos standalone]
	set sw_apps_dirs [lreverse $sw_apps_dirs]
	foreach dir $sw_apps_dirs {
	    return [glob -nocomplain -type {d  r} -path $dir "atf_sources"]
	}
    }
    #----------------------------------------------------------#
    # Find files with matching pattern in the base directory
    #----------------------------------------------------------#
    ...
    #--------------------------------------------------------------------------#
    # Find all the "sw_apps"/"sw_apps_linux" folders in the directory structure
    #--------------------------------------------------------------------------#
    proc find_repos { basedir } {
	variable repo_list

	foreach dirname [glob -nocomplain -type d -path $basedir *] {
	    set dirname $dirname/

	    # Recursively call the routine to find the directories in deep levels
	    find_repos $dirname
	    set name [file tail $dirname]

	    # If the directory is sw_apps / sw_apps_linux, update the list
	    if { $name == "sw_apps" || $name == "sw_apps_linux" } {
		lappend repo_list $dirname
	    }
	}
	return $repo_list
    }
    ...
    #--------------------------------------------------------------------------#
    proc get_all_repos { basedir os_dict } {
	foreach eachdir $basedir {
	    set dirlist [find_repos $eachdir]
	    foreach dir $dirlist {
		# Update the standalone apps dict
		if { [file tail $dir] == "sw_apps" } {
		    dict update os_dict standalone var {
			if { [lsearch $var $dir] == -1 } {
			    lappend var $dir
			}
		    }
		}
		# Update the Linux apps dict
		if { [file tail $dir] == "sw_apps_linux" } {
		    dict update os_dict linux var {
			if { [lsearch $var $dir] == -1 } {
			    lappend var $dir
			}
		    }
	...
	dict set osdict standalone "$sdk/data/embeddedsw/lib/sw_apps/"
	dict set osdict linux "$sdk/data/embeddedsw/lib/sw_apps_linux/"

	# Get apps directories from all the added repositories
	set os_apps_dict [get_all_repos $repopath $osdict]

	# Find all the files with TCL extensions in both standalone and linux directories
	dict for {os repo_list} $os_apps_dict {
	    foreach apps_repo $repo_list {
		set filelist [find_files $apps_repo "*.tcl"]
		# Create a namespace for each App and source the TCL file found
		foreach apptclpath $filelist {
		    set namespace_name [file rootname [file tail $apptclpath]]
		    namespace eval ::xsdb::sdk::$namespace_name [list source $apptclpath]
		    dict set os_app $namespace_name [dict create os $os path $apps_repo$namespace_name]
		}
	    }
	}
	return $os_app
	...
	# For all apps, the swapp_is_supported_hw is called from the application's tcl
	# For Peripheral Test App, the case is handled by function 'is_peri_test_supported_on_hw'
	if { $params(app) == "peripheral_tests" } {
	    is_peri_test_supported_on_hw $params(processor)
	} else {
	    ...
	    if { ($proc_ip_type == "psu_cortexr5" || $proc_ip_type == "ps7_cortexa9" || $proc_ip_type == "microblaze")  && $params(arch) == "64"} {
    ...
    #---------------------------------------------------------------------------------------#
    # Is Peripheral Test Supported on HW
    # Description: Check if the peripheral test is supported on hw
    #              Get all the memory ranges of the processor of type MEMORY
    #		   Peripheral Test app is not supported for PSU_MICROBLAZE
    # Arguments  : HW Design
    #---------------------------------------------------------------------------------------#
    proc is_peri_test_supported_on_hw { processor } {

	set type [::common::get_property IP_NAME [::hsi::get_cells $processor -hierarchical]]
	if {($type == "psu_pmu")} {
	    error "This application is not supported for PSU PMU processor (psu_pmu).";
	}

	# Typical test application size is 8KB
	set test_app_size 8192

	# Get all the memory ranges of the processor of type MEMORY
	set memlist [::hsi::get_mem_ranges -of_objects [::hsi::get_cells $processor -hierarchical ] -filter {MEM_TYPE == "MEMORY"}]

	# For any memory range, check if the size is greater than Test Application Size
	foreach mem $memlist {
	    set base [::common::get_property BASE_VALUE $mem]
	    set high [::common::get_property HIGH_VALUE $mem]
	    set size [expr $high - $base + 1]

	    if { [expr $size >= $test_app_size] } {
		return 1
	    }
	}
	error "Peripheral tests require atleast 8192 bytes of memory."
    }
	...
	foreach mdm $mdm_list {
		...
		for { set i 0 } { $i < $ports } { incr i } {
		    ...
		    set mdm_net [::common::get_property NAME [::hsi::get_intf_nets -of_objects $mdm_pins]]
	...
	[catch {file delete {*}[glob $dir/*.mss]} msg]
