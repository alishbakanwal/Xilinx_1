#ifndef XAXI_TLM_PAYLOAD
#define XAXI_TLM_PAYLOAD

namespace xaxi_tlm{
  class xaxis_tlm_extension;
  class xaximm_tlm_extension;

  struct xaxi_tlm_protocol_types{
    typedef tlm::tlm_generic_payload	tlm_payload_type;
    typedef tlm::tlm_phase				tlm_phase_type;
  };
  typedef xaxi_tlm_protocol_types::tlm_payload_type xaxi_tlm_transaction;
...
//      cout << __FUNCTION__ << " " << gp_data_ptr_location << endl;
    ...
    xaxis_tlm_extension* get_axis_extension(){
      return dynamic_cast<xaxis_tlm_extension*> (
          axi_transaction.get_extension(0)
          );
    }
    void set_axis_extension(xaxis_tlm_extension* ext){
      //extension object may not be set twice
      assert(axi_transaction.get_extension(0) == NULL);
      axi_transaction.set_extension(0,ext);
    }

    xaximm_tlm_extension* get_aximm_extension(){
      return dynamic_cast<xaximm_tlm_extension*> (
         (axi_transaction.get_extension<xaximm_tlm_extension>())
          );
    }
    void set_aximm_extension(xaximm_tlm_extension* ext){
      //extension object may not be set twice
      assert((axi_transaction.get_extension<xaximm_tlm_extension>()) == NULL);
      axi_transaction.set_extension(ext);
    }
    ...
    void set_id(unsigned int p_id){
      xaximm_tlm_extension* ext = get_aximm_extension();
      if(ext)
      {
        ext->trans_id = p_id;
      }
      ...
      {
        xaxis_tlm_extension* ext = get_axis_extension();
        if(ext)
        {
            ext->id = p_id;
        }
    ...
    xaxi_tlm_transaction axi_transaction;
