...
{\
  ...
  memcpy(\
        svcpp_name, \
        (IP_MODEL_INST.intf_name)->xaxis_skt_pin_if.get_pin(XAXIS_ENUM_##intf_pin_name).get_pVal(), \
        (length >> 3) + ((length%8 != 0) ? 1 : 0)\
        );\
}
...
{\
  unsigned int length = IP_MODEL_INST.pin_name->length();\
  memcpy(IP_MODEL_INST.pin_name->get_pVal(), \
         svcpp_name, \
         (length >> 3) + ((length%8 != 0) ? 1 : 0));\
}
...
{\
  ...
  memcpy(hdl_pin_name, \
    (IP_MODEL_INST.intf_name)->op_type##_##intf_type##_socket.xaximm_##op_type##_##data_type##_skt_pin_if.get_pin(XAXIMM_ENUM_##intf_pin_name).get_pVal(), \
    (length >> 3) + ((length%8 != 0) ? 1 : 0));\
}
...
{\
  ...
  memcpy(pa_inst->intf_name->op_type##_##intf_type##_socket.xaximm_##op_type##_##data_type##_skt_pin_if.get_pin(XAXIMM_ENUM_##intf_pin_name).get_pVal(),\
  hdl_pin_name,\
  (length >> 3) + ((length%8 != 0) ? 1 : 0));\
}
//---------------------------------------------------
