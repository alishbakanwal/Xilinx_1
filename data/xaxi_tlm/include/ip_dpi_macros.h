...
{\
  ...
  for(int i = 0; i < length/32;i++){\
    svcpp_name[i] = (IP_MODEL_INST(ip_name).pin_name)->range((i<<5)+31,(i<<5)).to_uint();\
  }\
  if(length%32 != 0){\
    unsigned int lo = ((length/32)*32);\
    unsigned int hi = length - 1;\
    unsigned int buf_idx   = (length/32);\
    svcpp_name[buf_idx] = (IP_MODEL_INST(ip_name).pin_name)->range(hi,lo).to_uint();\
  }\
}
...
{\
  ...
  for(int i = 0; i < length/32;i++){\
    hdl_pin_name[i] = (IP_MODEL_INST(ip_name).intf_name)->xaxis_skt_pin_if.get_pin(XAXIS_ENUM_##intf_pin_name).range((i<<5)+31,(i<<5)).to_uint();\
  }\
  if(length%32 != 0){\
    unsigned int lo = ((length/32)*32);\
    unsigned int hi = length - 1;\
    unsigned int buf_idx   = (length/32);\
    hdl_pin_name[buf_idx] = (IP_MODEL_INST(ip_name).intf_name)->xaxis_skt_pin_if.get_pin(XAXIS_ENUM_##intf_pin_name).range(hi,lo).to_uint();\
  }\
}
...
{\
  ...
  for(int i = 0; i < length/32;i+=1){\
    IP_MODEL_INST(ip_name).pin_name->range(int((i<<5)+31), int(i<<5)) = svcpp_name[i];\
  }\
  if(length%32 != 0){\
    ...
    (IP_MODEL_INST(ip_name).pin_name)->range(hi,lo) = value;\
  }\
}
...
{\
  unsigned int length = (IP_MODEL_INST(ip_name).intf_name)->xaxis_skt_pin_if.get_pin(XAXIS_ENUM_##intf_pin_name).length();\
  for(int i = 0; i < length/32;i+=1){\
    (IP_MODEL_INST(ip_name).intf_name)->xaxis_skt_pin_if.get_pin(XAXIS_ENUM_##intf_pin_name).range(int((i<<5)+31), int(i<<5)) = hdl_pin_name[i];\
  }\
  if(length%32 != 0){\
    unsigned int lo = ((length>>5)<<5);\
    unsigned int hi = length - 1;\
    ...
    (IP_MODEL_INST(ip_name).intf_name)->xaxis_skt_pin_if.get_pin(XAXIS_ENUM_##intf_pin_name).range(hi,lo) = value;\		}\
}
//---------------------------------------------------
//---------------------------------------------------
//AXIMM
...
{\
  ...
  for(int i = 0; i < length/32;i++){\
    hdl_pin_name[i] = (IP_MODEL_INST(ip_name).intf_name)->op_type##_##intf_type##_socket.xaximm_##op_type##_##data_type##_skt_pin_if.get_pin(XAXIMM_ENUM_##intf_pin_name).range((i<<5)+31,(i<<5)).to_uint();\
  }\
  if(length%32 != 0){\
    unsigned int lo = ((length/32)*32);\
    unsigned int hi = length - 1;\
    unsigned int buf_idx   = (length/32);\
    hdl_pin_name[buf_idx] = (IP_MODEL_INST(ip_name).intf_name)->op_type##_##intf_type##_socket.xaximm_##op_type##_##data_type##_skt_pin_if.get_pin(XAXIMM_ENUM_##intf_pin_name).range(hi,lo).to_uint();\
  }\
}
...
{\
  unsigned int length = (IP_MODEL_INST(ip_name).intf_name)->op_type##_##intf_type##_socket.xaximm_##op_type##_##data_type##_skt_pin_if.get_pin(XAXIMM_ENUM_##intf_pin_name).length();\
  for(int i = 0; i < length/32;i+=1){\
    (IP_MODEL_INST(ip_name).intf_name)->op_type##_##intf_type##_socket.xaximm_##op_type##_##data_type##_skt_pin_if.get_pin(XAXIMM_ENUM_##intf_pin_name).range(int((i<<5)+31), int(i<<5)) = hdl_pin_name[i];\
  }\
  if(length%32 != 0){\
    unsigned int lo = ((length>>5)<<5);\
    unsigned int hi = length - 1;\
    unsigned int buf_idx   = (length/32);\
    xsc_bv value(length%32,hdl_pin_name[buf_idx]); \
    (IP_MODEL_INST(ip_name).intf_name)->op_type##_##intf_type##_socket.xaximm_##op_type##_##data_type##_skt_pin_if.get_pin(XAXIMM_ENUM_##intf_pin_name).range(hi,lo) = value;\		}\
}
//---------------------------------------------------
