...
namespace xaxi_tlm{
  class xaximm_protocol_extension:
    public tlm::tlm_extension<xaximm_protocol_extension>{
    public:
      xaximm_protocol_extension():tlm::tlm_extension<xaximm_protocol_extension>(){
      beat_no = 0;
      addr = 0;
      Aligned_Address = 0;
      aligned = false;
      ...
      virtual tlm::tlm_extension_base * clone() const{
        xaximm_protocol_extension* cl = new xaximm_protocol_extension();
        cl->copy_from(*this);
        return (cl);
      }
      virtual void copy_from(tlm::tlm_extension_base const & ext){
        sc_assert(ID == static_cast<xaximm_protocol_extension const &>(ext).ID);
        operator =(static_cast<xaximm_protocol_extension const &>(ext));

        xaximm_protocol_extension const& xaximm_ext = (static_cast<xaximm_protocol_extension const &>(ext));
